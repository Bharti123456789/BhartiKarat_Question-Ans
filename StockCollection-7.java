...
class StockCollection {
    ...
    //In testStockCollection(), the very first checks are:
   // Now when stockCollection is empty:
    //isEmpty() is true → returns the expected values: -1,-1 ,-1.0
    //And when records exist, it uses the stream calculations normally.
    int getMaxPrice() {
        /** Return the maximum price recorded in this StockCollection. */
        if (priceRecords.isEmpty()) return -1;//fix issue
    ...
    double getAvgPrice() {
        ...
        if (priceRecords.isEmpty()) return -1.0; // fix issue
